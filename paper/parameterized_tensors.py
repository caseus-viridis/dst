...
class SparseTensor(nn.Module):
    def __init__(self,tensor_size,initial_sparsity,sub_kernel_granularity = False):
        ...
        trailing_dimensions = tensor_size[2:] if sub_kernel_granularity else  []
        self.register_buffer('mask',torch.Tensor(*tensor_size))
        ...
        if self.conv_tensor and not(self.sub_kernel_granularity):
            self.tensor_sign = torch.sign(self.s_tensor.sum(-1).sum(-1).view(-1))
        else:
    ...
    def prune_sign_change(self,reinitialize_unused_to_zero = True,enable_print = False):
        if self.conv_tensor and not(self.sub_kernel_granularity):
            W_flat = self.s_tensor.abs().sum(-1).sum(-1).view(-1)
        else:
    ...
    def prune_small_connections(self,prune_fraction,reinitialize_unused_to_zero = True):
        ...
        cutoff = int(prune_fraction * W_masked.numel())
    ...
    def get_sparsity(self):
        active_elements = self.mask.sum() * (np.prod(self.s_tensor.size()[2:]).item() if self.conv_tensor and not(self.sub_kernel_granularity) else 1)
        return (active_elements,1 - active_elements / self.s_tensor.numel())


    def forward(self):
        if self.conv_tensor:
            if self.sub_kernel_granularity:
                return self.mask * self.s_tensor
            else:
                return self.mask.unsqueeze(-1).unsqueeze(-1) * self.s_tensor
        else:
            return self.mask * self.s_tensor
