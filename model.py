...
class mnist_mlp(DynamicNetworkBase):
    ...
    def __init__(self,  initial_sparsity = 0.98,sparse = True):
        super(mnist_mlp, self).__init__()

        self.fc1 = DynamicLinear(784, 300, initial_sparsity,bias = False,sparse = sparse)
        self.fc_int = DynamicLinear(300, 100, initial_sparsity,bias = False,sparse = sparse)
        self.fc2 = DynamicLinear(100, 10, initial_sparsity,bias = False,sparse = sparse)
...
class cifar10_WideResNet(DynamicNetworkBase):
    def __init__(self, depth, num_classes=10, widen_factor=1, dropRate=0.0,initial_sparsity_conv = 0.5,initial_sparsity_fc = 0.95,sub_kernel_granularity = False,sparse = True):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,widen_factor = 1,vanilla_conv1 = True,vanilla_conv3 = True,initial_sparsity = 0.5,
                 sub_kernel_granularity = False,sparse = True):
...
class ResNet(DynamicNetworkBase):
    ...
    def __init__(self, block, layers, num_classes=1000,widen_factor = 1,vanilla_downsample = True,vanilla_conv1 = True,vanilla_conv3 = True,
                 initial_sparsity_conv = 0.5,initial_sparsity_fc = 0.95,sub_kernel_granularity = False,sparse = True):
...
def imagenet_resnet50(pretrained=False, widen_factor = 1,vanilla_conv1 = False,vanilla_conv3 = False,vanilla_downsample = True,decimation_factor = 8,
                      initial_sparsity_conv = 0.5,initial_sparsity_fc = 0.95,sub_kernel_granularity = False,sparse = True, **kwargs):
